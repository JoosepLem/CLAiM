...
Hierarchical abstractions and diagrams for software architecture static structure. Diagrams in **Mermaid C4 syntax**.
...
**Titles**: `"[Diagram Type] for [Subject][ - Environment]"`. Always present. Include key/legend when using >1 element type or relationship style.
...
One `system-context.mmd` and `containers.mmd` per system. One `components.mmd` per container subdirectory (kebab-case, matching Mermaid ID). One `deployment-<env>.mmd` per environment (production required, staging/dev optional). All `.mmd` files contain a ` ```mermaid ` block — diffable, GitHub-rendered.
...
**Rendering**: GitHub renders `.mmd` natively. CLI export: `npx -p @mermaid-js/mermaid-cli mmdc -i in.mmd -o out.png`.
...
4. **Deployment**: Map to infra → `deployment/deployment-production.mmd`
