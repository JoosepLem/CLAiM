...
const TYPE: Record<Discrepancy['type'], { short: string; color: string; bg: string }> = {
  missing: { short: 'Missing', color: '#B0492F', bg: '#F4E7E0' },
  qty: { short: 'Qty mismatch', color: '#8A6A1E', bg: '#F3EBD7' },
};
...
interface AllRow {
  ...
  statusShort: string; flagged: boolean; barColor: string;
}
...
export default function ReconciliationReport() {
  ...
  const orderedRows = useMemo<AllRow[]>(() => {
    const disc = DISCREPANCIES.map((d) => ({
      ...
      statusShort: TYPE[d.type].short, flagged: true, barColor: TYPE[d.type].color, line: d.line,
    }));
    const matched = MATCHED.map((m) => ({
      ...
      statusShort: 'Matched', flagged: false, barColor: 'transparent', line: m.line,
    }));
        ...
        <div className="bg-cream-card border border-line rounded-[14px] shadow-[0_1px_3px_rgba(40,30,10,0.07),0_12px_28px_-16px_rgba(40,30,10,0.18)] overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-[#ECE6D8] bg-cream-panel">
            <div className="flex items-center gap-3.5">
              ...
              <span className="text-[14px] text-[#6E6A5E] font-medium">Reconciliation report</span>
            </div>
            <span className="inline-flex items-center gap-1.5 bg-[#F3EBD7] text-[#8A6A1E] px-3 py-1.5 rounded-full font-semibold text-[12.5px]">
              <span className="w-[7px] h-[7px] rounded-full bg-gold inline-block" /> Needs review
            </span>
          </div>
          ...
          <div className="flex items-start justify-between gap-6 px-6 pt-[22px] pb-5">
            <div className="flex-1">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <div className="text-[13px] text-muted mt-0.5">Laboratory services · Partner invoice</div>
                </div>
              </div>
              <div className="flex gap-[34px] mt-5">
                {[
                  ...
                  ['Uploaded', uploaded],
                  ['Lines', String(REPORT_META.totalLines)],
                ].map(([k, v]) => (
                  <div key={k}>
            ...
            <button className="inline-flex items-center gap-2 bg-ink text-cream-card rounded-[10px] px-[17px] py-[11px] text-[14px] font-semibold whitespace-nowrap hover:bg-[#2C3E4F] transition-colors">
              <Download size={15} /> Download partner PDF
            </button>
          </div>
          ...
          <div className="grid grid-cols-[1.25fr_1fr_1fr] gap-3.5 px-6 pb-[22px]">
            <div className="bg-[#FBF1EC] border border-[#EBD7CC] rounded-xl px-[18px] py-[15px]">
              <div className={`${microLabel} text-danger`}>Money at risk</div>
              <div className="font-mono text-[30px] font-semibold text-danger mt-1.5 tracking-[-0.01em]">{money(kpi.moneyAtRisk)}</div>
              <div className="text-[12.5px] text-[#A06B52] mt-0.5">across {kpi.remaining} open discrepancies</div>
            </div>
            <div className="bg-white border border-line rounded-xl px-[18px] py-[15px]">
              <div className={`${microLabel} text-[#9A9484]`}>Discrepancies left</div>
              <div className="font-mono text-[30px] font-semibold mt-1.5 tracking-[-0.01em]">{kpi.remaining}</div>
              <div className="text-[12.5px] text-muted mt-0.5">of {kpi.total} flagged · {kpi.resolvedCount} resolved</div>
            </div>
            <div className="bg-white border border-line rounded-xl px-[18px] py-[15px]">
              <div className={`${microLabel} text-[#9A9484]`}>Lines reconciled</div>
          ...
          {tab === 'all' && (
            <div className="px-[18px] pt-3.5 pb-5 bg-[#F6F3EA]">
              <div className="text-[12.5px] text-muted px-1 pb-3">
                Showing {orderedRows.length} representative lines of {REPORT_META.totalLines} · flagged lines highlighted
              </div>
              <div className="border border-line rounded-[10px] overflow-hidden bg-white">
                <div className="grid grid-cols-[118px_64px_84px_86px_1fr_44px_84px_92px] gap-2 px-3.5 py-2.5 bg-[#F3EFE4] border-b border-[#E6DFCF] font-mono text-[10px] tracking-[0.06em] uppercase text-[#9A9484]">
                  <span>Status</span><span>Date</span><span>Patient</span><span>Code</span><span>Description</span>
                  <span className="text-right">Qty</span><span className="text-right">Unit</span><span className="text-right">Total</span>
                </div>
                {orderedRows.map((r) => (
                  <div
                    key={r.id}
                  ...
                  >
                    <span><span className="inline-block font-mono text-[10px] font-semibold px-2 py-[3px] rounded-[5px] whitespace-nowrap" style={{ background: r.flagged ? statusBg(r.statusShort) : '#E6EFE8', color: statusColor(r.statusShort) }}>{r.statusShort}</span></span>
...
function statusBg(s: string) {
  if (s === 'Missing') return '#F4E7E0';
  if (s === 'Qty mismatch') return '#F3EBD7';
  return '#E6EFE8';
}
function statusColor(s: string) {
  if (s === 'Missing') return '#B0492F';
  if (s === 'Qty mismatch') return '#8A6A1E';
  return '#3C7355';
}

function DiscCard({
  d, expanded, resolved, onToggle, onResolve,
}: {
  d: Discrepancy;
  expanded: boolean;
  resolved: boolean;
    ...
    >
      <div className="flex items-center gap-3.5 px-3.5 py-3.5">
        ...
        <div className="text-right mr-0.5">
          ...
          <div className="font-mono text-[10.5px] text-[#A39C8A] tracking-[0.04em] uppercase">at risk</div>
        </div>
        <button
          onClick={onResolve}
      ...
      {expanded && (
        <div className="border-t border-[#ECE6D8] bg-[#FBF9F3] p-4 grid grid-cols-2 gap-3.5">
          {present && (
            <>
              <CompareCard title="Partner invoice — Synlab" rows={[
                ['Service', d.name], ['Code', d.code], ['Quantity', String(d.pq)], ['Line total', money(d.price * d.pq)],
              ]} />
              <CompareCard title="Patient invoice — to insurer" rows={[
                ['Service', d.name], ['Code', d.code], ['Quantity', String(d.ptq)], ['Line total', money(d.price * d.ptq)],
              ]} highlightQty />
            </>
          )}
          ...
          <div className="col-span-2 border border-[#E2D5B6] bg-[#FAF4E6] rounded-[10px] px-4 py-3.5 flex items-center gap-5 flex-wrap">
            <div className="min-w-[150px]">
              <div className="font-mono text-[10.5px] tracking-[0.06em] uppercase text-[#8A6A1E] font-semibold">Add to patient invoice</div>
              <div className="text-[12px] text-[#8C7B57] mt-0.5 leading-snug max-w-[210px]">Patient invoices live in the billing service. Use these to find the encounter.</div>
            </div>
            <div className="flex items-start gap-6 ml-auto flex-wrap">
              <div>
                <div className="font-mono text-[9.5px] tracking-[0.06em] uppercase text-[#A88F5A]">Patient ID</div>
                <div className="font-mono text-[15px] font-semibold mt-0.5">{d.patient}</div>
              </div>
              <div>
                <div className="font-mono text-[9.5px] tracking-[0.06em] uppercase text-[#A88F5A]">Recent patient invoices</div>
          ...
          <div className="col-span-2 bg-ink rounded-[10px] px-3.5 py-3">
            <div className="flex justify-between items-center mb-2.5">
              <span className="font-mono text-[10.5px] tracking-[0.06em] uppercase text-[#A9B4BF]">Source · Synlab PDF — {pdfRef}</span>
              <span className="text-[12.5px] text-[#C9A95F] font-semibold cursor-pointer">Open full PDF →</span>
            </div>
    ...
    <div className="border border-[#E6DFCF] rounded-[10px] bg-white px-[15px] py-3.5">
      ...
      {rows.map(([k, v], i) => (
        <div key={k} className="flex justify-between text-[13px] py-1.5" style={{ borderBottom: i < rows.length - 1 ? '1px solid #F0EADC' : 'none' }}>
          ...
          <span className={`text-right ${k === 'Quantity' || k === 'Line total' ? 'font-mono font-semibold' : ''}`}
            style={highlightQty && k === 'Quantity' ? { color: '#B0492F' } : undefined}>
            {v}
          </span>
        </div>
      ))}
